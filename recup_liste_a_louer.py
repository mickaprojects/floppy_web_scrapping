...
if os.path.exists("recup_liste_a_louer.lock")==False:
    try:
        lock=open("recup_liste_a_louer.lock", "a")
        lock.close()
        k = 0

        #31 12 2018 python27
        trace = open("trace_recup_liste_a_louer.txt", "w")
        trace.close()
        ...
        if (os.access(nom_parametre, os.F_OK) == False):
            trace = open("trace_recup_liste_a_louer.txt", "a")
        ...
        trace = open("trace_recup_liste_a_louer.txt", "a")
        trace.write("FIN Recup liste a louer "+categorie+" !"+"\n")
        trace.close()

        #Suppression du fichier .lock
        if os.path.exists('recup_liste_a_louer.lock')==True:
            os.remove('recup_liste_a_louer.lock')
