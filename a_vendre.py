...
if os.path.exists("a_vendre.lock")==False:
    try:
        lock=open("a_vendre.lock", "a")
        lock.close()
        k = 0

        #31 12 2018 python27
        trace = open("trace_a_vendre.txt", "w")
        trace.close()
        ...
        if (os.access(nom_parametre, os.F_OK) == False):
            trace = open("trace_a_vendre.txt", "a")
        ...
        curlocal.execute("select * from table_recup_liste_a_vendre where bien<>'Terrains' and bien<>'Garages' and categorie <> 'construire' and categorie<>'vacance' and categorie<>'agence' and traite='n'")
        t=curlocal.fetchall()
        z=0
        for enr in t:
            z=z+1
            ...
            if z==1:
                ...
                export = open(rep + "\\" + str(date_)+".txt", "w")
            ...
            export = open(rep + "\\" + str(date_)+".txt", "a")
        ...
        if os.path.exists('a_vendre.lock')==True:
            os.remove('a_vendre.lock')
