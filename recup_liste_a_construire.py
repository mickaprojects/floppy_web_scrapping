...
if os.path.exists("recup_liste_a_construire.lock")==False:
    try:
        lock=open("recup_liste_a_construire.lock", "a")
        lock.close()
        k = 0

        #31 12 2018 python27
        trace = open("trace_recup_liste_a_construire.txt", "w")
        trace.close()
        ...
        if (os.access(nom_parametre, os.F_OK) == False):
            trace = open("trace_recup_liste_a_construire.txt", "a")
        ...
        while True:
            ...
            try:
                ...
                # if page1=="2":
                #     current_url=href
                #     url_suivant=current_url.replace("?page=2", "?page=4")
        ...
        trace = open("trace_recup_liste_a_construire.txt", "a")
        trace.write("FIN Recup liste a construire "+categorie+" !"+"\n")
        trace.close()

        #Suppression du fichier .lock
        if os.path.exists('recup_liste_a_construire.lock')==True:
            os.remove('recup_liste_a_construire.lock')
